...
### Functions ######### ----
### 
# Function to return full table of weighted percentages by group and filtered to one value of a variable:
...
weight_perc_2groups <- function(df, var, weight, group1, group2, filter_value = NULL) {
    # If filter_value is not NULL:
    if (!is.null(filter_value)) {
    # Get percent for group1 (usually county):
    group1_total <-  df %>% select({{weight}}, {{var}}, {{group1}}) %>% group_by(!!sym((group1))) %>% count(!!sym((var)), wt = !!sym((weight))) %>% mutate(total_pop = sum(n)) %>% 
...
transport_mean_scag_table <- transport_mean_scag_table %>% mutate(across(where(is.numeric), round, 2))
...
# Full join all scag_race tables, then left join scag_total_no_veh_table by county:
med_hr_wage_scag_table <- full_join(scag_county_race_med_hr_wage,scag_race_med_hr_wage) %>% left_join(.,scag_total_med_hr_wage_table, by = join_by(county))
...
# Full join all scag_race tables, then left join scag_total_no_veh_table by county:
unemployed_scag_table <- full_join(scag_county_race_unemployed,scag_race_unemployed) %>% left_join(.,scag_total_unemployed_table, by = join_by(county))
...
# Full join all scag_total tables:
total_pop_scag_table <- full_join(scag_county_total_pop, scag_total_pop, by = join_by(county, total_pop))
...
# Full join all scag_total tables:
race_perc_scag_table <- left_join(scag_race_perc_county, scag_race_perc_total, by = join_by(race)) %>% ungroup()
...
# Full join all scag_total tables:
med_hh_income_scag_table <- full_join(scag_med_hh_income_county, scag_med_hh_income_total, by = join_by(med_hh_income, race)) %>% 
                                        arrange(housing_income) %>% ungroup() %>% rename(housing_incomecation_level = housing_income)
...
# 
# Pass in data filtered to only hh (filter(RELSHIPP == 20)), calculate weighted percentages using hh level weights- "WGTP", for variable "poverty" and filter to "poverty" == "Yes": 
poverty_hh_scag_table <- ca_pums_scag %>% filter(RELSHIPP == 20) %>% weight_perc_table_filter(var = "poverty", weight = "WGTP", group = "race", filter_value = "Yes")
