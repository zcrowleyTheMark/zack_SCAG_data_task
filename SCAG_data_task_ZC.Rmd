...
```{r fig 20}
### 
# TODO: NOT Matching up with figure 20 from EA report and figure workbook...
# 
# Figure 20. People Living in Households Without Kitchen and Plumbing Facilities by Race and Ethnicity
...
scag_race_no_kitchen_plumbing <- ca_pums_scag %>% filter(RELSHIPP == 20) %>% select(WGTP, race, no_kitchen_plumbing) %>%  
                                         group_by(race) %>% 
                                         summarize(
                                           total_pop = sum(WGTP),
...
scag_county_race_no_kitchen_plumbing <- ca_pums_scag %>% filter(RELSHIPP == 20) %>% select(WGTP, race, no_kitchen_plumbing, county) %>%  
                                         group_by(county, race) %>% 
                                         summarize(
                                           total_pop = sum(WGTP),
                                           no_kitchen_plumbing_sum = sum(WGTP[no_kitchen_plumbing == "Yes"]), # sum up only those that own homes.
...
scag_county_total_no_kitchen_plumbing <- ca_pums_scag %>% filter(RELSHIPP == 20) %>% select(WGTP, county, no_kitchen_plumbing) %>%  
                                         group_by(county) %>% 
                                         summarize(
                                           total_pop = sum(WGTP),
                                           no_kitchen_plumbing_sum = sum(WGTP[no_kitchen_plumbing == "Yes"]), # sum up only those that own homes.
...
scag_total_no_kitchen_plumbing <- ca_pums_scag %>% filter(RELSHIPP == 20) %>% select(WGTP, no_kitchen_plumbing) %>%  
                                        summarize(
                                           total_pop = sum(WGTP),
                                           no_kitchen_plumbing_sum = sum(WGTP[no_kitchen_plumbing == "Yes"]), # sum up only those that own homes.
                                           no_kitchen_plumbing_prop = no_kitchen_plumbing_sum / total_pop, # divide by total_pop for proportion
...
# Table of contents for output "scag_tabbed.xlsx":
...
sheets <- list("TOC" = toc,
               ...
               "Figure_22" = scag_homeownership_table) # "scag_homeownership_table" is a tibble that will populate the sheet named "Figure_22".
