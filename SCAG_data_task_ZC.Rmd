...
ca_pums_scag <- ca_pums_scag %>% mutate(race = case_when(HISP == 1 & RAC1P == 1 ~ "White", 
...
homeownership_stats <- ca_pums_scag %>% group_by(county, race) %>% 
                                        summarize(
                                          total_pop = sum(PWGTP),
                                          ownership = sum(PWGTP[own_home == "Homeowner"]), # sum up only those that own homes.
                                          ownership_pct = sum(PWGTP[own_home == "Homeowner"]) / sum(PWGTP) # divide by total_pop for percent
                                        )
```
