...
scag_county_race_unemployed <- ca_pums_scag %>%  select(PWGTP, county, race, unemployed,ESR) %>% drop_na(unemployed) %>%
                                         ...
                                         pivot_wider(names_from = race, values_from = unemployed_perc)
...
scag_county_total_unemployed <- ca_pums_scag %>% select(PWGTP, county, unemployed,ESR) %>% drop_na(unemployed) %>%
                                               ...
                                               summarize(
                                                       ...
                                                       unemployed_prop = unemployed_sum / total_labor, # divide by total_labor for proportion
                                                       unemployed_perc = round(unemployed_prop*100, 2)
                                                     ) %>% select(county, unemployed_perc) %>% rename(Total = unemployed_perc)
...
scag_total_unemployed <- ca_pums_scag %>%  select(PWGTP, county, unemployed,ESR) %>% drop_na(unemployed) %>%
                                               summarize(
                                                       ...
                                                       unemployed_prop = unemployed_sum / total_labor, # divide by total_labor for proportion
                                                       unemployed_perc = round(unemployed_prop*100, 2)
                                                     ) %>% mutate(county = "SCAG") %>% select(county, unemployed_perc) %>% rename(Total = unemployed_perc)

# Full join all scag_total tables:
scag_total_unemployed_table <- full_join(scag_county_total_unemployed,scag_total_unemployed)

# Full join all scag_race tables, then left join scag_total_no_veh_table by county:
scag_unemployed_table <- full_join(scag_county_race_unemployed,scag_race_unemployed) %>% left_join(.,scag_total_unemployed_table, by = join_by(county))
scag_unemployed_table

# Write out table to .csv in "output/tables" folder and once the file exists remove from current environment all tibbles used to make the final table:
write_csv(scag_unemployed_table, here(tables_fp, "scag_unemployed_table.csv"))
if (file.exists(here(tables_fp, 'scag_unemployed_table.csv'))) {rm(list = ls(pattern = "_unemployed"))}
...
# Full join all scag_race tables, then left join scag_total_no_veh_table by county:
scag_working_poor_table <- full_join(scag_county_race_working_poor,scag_race_working_poor) %>% left_join(.,scag_total_working_poor_table, by = join_by(county))
scag_working_poor_table

# Write out table to .csv in "output/tables" folder and once the file exists remove from current environment all tibbles used to make the final table:
write_csv(scag_working_poor_table, here(tables_fp, "scag_working_poor_table.csv"))
if (file.exists(here(tables_fp, 'scag_working_poor_table.csv'))) {rm(list = ls(pattern = "_working_poor"))}
...
scag_total_pop <- ca_pums_scag %>% select(PWGTP) %>%  
                                         summarize(
                                           total_pop = sum(PWGTP)
                                         ) %>% mutate(county = "SCAG") %>% select(county, total_pop)
...
# Full join all scag_total tables:
scag_total_pop_table <- full_join(scag_county_total_pop, scag_total_pop, by = join_by(county, total_pop))
scag_total_pop_table

# Write out table to .csv in "output/tables" folder and once the file exists remove from current environment all tibbles used to make the final table:
write_csv(scag_total_pop_table, here(tables_fp, "scag_total_pop_table.csv"))
if (file.exists(here(tables_fp, 'scag_total_pop_table.csv'))) {rm(list = ls(pattern = "_total_pop"))}
...
scag_race_perc_total <- ca_pums_scag %>% select(PWGTP, race) %>% count(race, wt = PWGTP) %>% mutate(total_pop = sum(n)) %>% 
                                        group_by(race) %>% summarize(SCAG = round( (n / total_pop)*100, 2))
...
scag_race_perc_county <- ca_pums_scag %>% select(PWGTP, race, county) %>% group_by(county) %>% count(race, wt = PWGTP) %>% mutate(total_pop = sum(n)) %>% ungroup() %>% 
                                         group_by(race, county) %>% summarize(race_perc = round( (n / total_pop)*100, 2)) %>% 
                                         pivot_wider(names_from = county, values_from = race_perc)

# Full join all scag_total tables:
scag_race_perc_table <- left_join(scag_race_perc_county, scag_race_perc_total, by = join_by(race)) %>% ungroup()
scag_race_perc_table

# Write out table to .csv in "output/tables" folder and once the file exists remove from current environment all tibbles used to make the final table:
write_csv(scag_race_perc_table, here(tables_fp, "scag_race_perc_table.csv"))
if (file.exists(here(tables_fp, 'scag_race_perc_table.csv'))) {rm(list = ls(pattern = "_race_perc"))}
...
scag_age_perc_table <- left_join(scag_age_perc_county, scag_age_perc_total, by = join_by(age_3_cat)) %>% ungroup() %>%
                                pivot_longer(cols = Imperial:SCAG, names_to = "county", values_to = "percentages") %>% rename(age_categories = age_3_cat)

# Write out table to .csv in "output/tables" folder and once the file exists remove from current environment all tibbles used to make the final table:
write_csv(scag_age_perc_table, here(tables_fp, "scag_age_perc_table.csv"))
if (file.exists(here(tables_fp, 'scag_age_perc_table.csv'))) {rm(list = ls(pattern = "_age_perc"))}
...
# Full join all scag_total tables, and pivot_longer() to have county three columns: age_categories, county, and corresponding percentages:
scag_single_hh_table <- left_join(scag_single_hh_county, scag_single_hh_total, by = join_by(single_hh)) %>% ungroup() %>%
                                pivot_longer(cols = Imperial:SCAG, names_to = "county", values_to = "percentages") %>% 
                                rename(single_parent_households = single_hh)
scag_single_hh_table
# Write out table to .csv in "output/tables" folder and once the file exists remove from current environment all tibbles used to make the final table:
write_csv(scag_single_hh_table, here(tables_fp, "scag_single_hh_table.csv"))
if (file.exists(here(tables_fp, 'scag_single_hh_table.csv'))) {rm(list = ls(pattern = "_single_hh"))}
...
sheets <- list("TOC" = toc,
               ...
               "Figure_40" = scag_unemployed_table,
               "Figure_41" = scag_working_poor_table,
               "Total_Population" = scag_total_pop_table,
               "Race_Ethnicity" = scag_race_perc_table,
               "Age" = scag_age_perc_table,
               "Singe_Parent_HHs" = scag_single_hh_table) 
