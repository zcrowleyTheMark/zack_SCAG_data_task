...
pums_vars_acs5 <- read_csv("data_dicts_ACS_PUMS/pums_vars_acs5.csv", show_col_types = FALSE)
pums_vars_acs5_person <- read_csv("data_dicts_ACS_PUMS/pums_vars_acs5_person.csv", show_col_types = FALSE)
pums_vars_acs5_housing <- read_csv("data_dicts_ACS_PUMS/pums_vars_acs5_housing.csv", show_col_types = FALSE)
...
pums_vars_acs5_22 <- read_csv("data_dicts_ACS_PUMS/acs_data_dict_22.csv", show_col_types = FALSE)
...
```{r ACS_data_new_vars_tbl, cols.print=4}
...
ACS_data_new_vars_tbl <- tribble(~ variable_name,           ~ description,       ~ type,   ~ levels,
                                ...
                                "factor", c("female_hh", "male_hh","cohabit_or_nokids"),
                                 "lep", "3 categories of English Proficiency: Limited English Proficiency, English Proficient or Younger than 5 yos or no English spoken", 
                                "factor", c("limited_english_proficiency","english_proficient","<5yos_or_no_eng")
                                 )
...
write_csv(ACS_data_new_vars_tbl, "data_dicts_ACS_PUMS/ACS_data_new_vars_tbl.csv")
...
```{r ACS 2021 FTP data, eval=FALSE, include=FALSE}
...
# Write out to .csv for the raw data from ACS Data call to folder named "clean_data/acs_data_21":  881207 obs.  with 34 variables
...
```{r ACS 2022 FTP data, eval=FALSE, include=FALSE}
...
### Recode factor variables for analysis -----
# Use the levels that were set to the current working environment in code chuck "ACS_data_new_vars_tbl" and set all variables to factors:
ca_pums_scag <- ca_pums_scag %>% mutate(county = factor(county, levels = county_levels),
                                        race = factor(race, levels = race_levels),
                                        age_3_cat = factor(age_3_cat, levels = age_3_cat_levels),
                                        edu = factor(edu, levels = edu_levels),
                                        own_home = factor(own_home, levels = own_home_levels),
                                        walk_bike_public_transport = factor(walk_bike_public_transport, levels = walk_bike_public_transport_levels),
                                        all_transport = factor(all_transport, levels = all_transport_levels),
                                        poverty = factor(poverty, levels = yes_no_levels),
                                        housing_burden = factor(housing_burden, levels = yes_no_levels),
                                        renter_burden = factor(renter_burden, levels = yes_no_levels),
                                        no_kitchen_plumbing = factor(no_kitchen_plumbing, levels = yes_no_levels),
                                        overcrowded = factor(overcrowded, levels = yes_no_levels),
                                        unemployed = factor(unemployed, levels = yes_no_levels),
                                        single_hh = factor(single_hh, levels = single_hh_levels),
                                        lep = factor(lep, levels = lep_levels),
                                        ) # end of mutate for ca_pums_scag to factor variables
...
# county_levels: c("Imperial", "Los Angeles", "Orange", "Riverside", "San Bernardino", "Ventura", "SCAG")
if (county_levels[7] != "SCAG") {append(county_levels, "SCAG")}

# race_levels: c("Asian/Pacific Islander", "Black", "Hispanic/Latino", "Multiracial/Other", "Native American", "White", "All Races")
if (race_levels[7] != "SCAG") {append(race_levels,  "All Races")}
...
- Each of the following code chunks will create a table and write it out to a .csv file to the folder `output/tables`.
